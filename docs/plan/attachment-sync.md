...
附件现在是 submit → fan-out → deliver 协议的一等成员，并有严格生命周期，保证二进制数据不超出其投递窗口存活。
...
- 不重写用户当纯文本粘贴的远程 URL；除 provider 原生把长文本 paste 转成附件的场景外，ask'em 不把普通文本自行升级成附件。
- 不复用 provider 服务端已存文件句柄；每个 target 各自上传。
- 不劫持 provider 自己的 XHR 上传。
- 不跨浏览器 session 持久化附件。
- 不做格式转换、压缩、远程 fetch、same-file dedupe。

## Original Baseline（text-only 链路）

这些是附件同步实现前的约束，保留在这里解释为什么需要 ref/store/lifecycle 设计：
...
File extraction 不依赖 `instanceof File`：content script isolated world 与页面 MAIN world 之间可能出现跨 realm File-like 对象，需按 `name/type/size/arrayBuffer` duck-typing 接受。

若 capture buffer 为空，submit 前可从 provider-scoped `input[type=file]` 读取当前 `files` 做 late capture，然后仍走 source snapshot gate。没有当前 preview/card 时 fail-closed，不信任 stale `input.files`。

不特殊处理 HTML-with-inline-image 粘贴：这些 composer 实测只保留纯文本，ask'em 镜像该原生行为，不解析 `<img>`。

部分 provider 会把超长纯文本 paste 原生转成一个“pasted text”附件。ask'em 对已验证的 provider 做 provider-specific 捕获与 submit-time DOM 确认；没有验证的 provider 不用全局启发式猜测。
...
adapter 必须能完全 override payload 顺序。多数 target 可 text-first；Gemini 等需要 attach-first 时由 adapter 自己控制，core 不假设统一顺序。
...
| Provider | 目标注入路径 | 正向确认信号 | 主要风险 | 收口条件 |
| --- | --- | --- | --- | --- |
...
### 10. Source truth：capture buffer 只是 byte source，submit-time DOM 才是当前真相

附件 capture 发生在 paste/drop/input change/transient input 时，但用户可能在真正发送前删除、替换、或让 provider 把长文本 paste 转成自己的 attachment chip。实现上不把 capture buffer 当作“待发送附件列表”的权威来源：

- capture buffer 只负责保留可重放的 `File` bytes 和稳定 id。
- submit 前读取 provider-scoped `getComposerAttachmentSnapshot()`，把当前 DOM 的 chip/preview 与 buffer 匹配。
- 匹配不到、数量不一致、同名重复导致无法判断时，跳过本次附件 fan-out，并 toast 提示；不猜、不发旧 buffer、不阻断源 provider 原生发送。
- buffer 在 submit payload 被消费后清空，避免旧附件污染下一次 prompt。

### 11. Core/provider split：脏 DOM 留在 adapter，core 只管协议与生命周期

当前实现刻意把 provider 差异压在 adapter：

- source snapshot、target presence key、upload error selector、payload 顺序、transient menu 操作都在 provider adapter 内。
- core delivery 只依赖 `prepareForDelivery` / `setComposerPayload` / `getComposerAttachmentPresence` / `detectAttachmentUploadError` 这些抽象 hook。
- generic source snapshot 是 opt-in fallback，不作为默认猜测；不确定时 fail closed。
- Manus 这类特殊 provider 可以在 adapter 里特殊处理，但不把 `if manus` 扩散进 background/content core。

### 12. Failure model：source 可降级，target 不静默降级

源页面已经由用户原生点击发送，ask'em 不能阻断源 provider：

- source-side staging 失败（超预算、过大、IndexedDB 写失败等）→ 跳过附件 fan-out，仍发 `USER_SUBMIT` 文本，UI 提示附件同步跳过。
- target-side 附件注入/确认失败 → 该 target 不 submit 纯文本，返回 `upload-failed` / `delivery-failed`。
- target 数量超能力（例如 Manus free plan 1 file）→ 只跳过该 target，其他 target 继续；不发送前 N 个，也不发送纯文本。
- provider 原生拒绝格式/大小 → 不在 background 预测，交给 target adapter 检测并映射为 `upload-failed`。

### 13. Programmatic echo suppression is part of attachment safety

target 注入会触发 DOM/input/change/paste 等事件，不能被 source-capture 逻辑反射成“用户又提交了一次”。所以 delivery 开始前就开启两层 suppression：

- submit observation suppression：避免程序化 submit 被当作用户 submit fan-out 回来。
- attachment capture suppression：避免 target 注入时的 file input/change 被 capture buffer 当作 source 附件。

窗口覆盖整个 attachment delivery timeout + buffer，宁可多 suppress 一小段时间，也不允许程序注入造成递归 fan-out。

### 14. Debug logs：准确但有大小边界

日志用于开发期定位真实 provider 行为，不能把 prompt preview、filename key、DOM signal、失败原因替换成无用占位。但日志也不能无限增长：

- 本地 debug log 是 tail buffer：entry count + serialized bytes 双限制。
- 单条 message/detail 过长时做 middle truncation，保留头尾和明确截断标记。
- feedback 附带 logs 再用更小预算二次裁剪，避免 bug report 发送超大 payload。
- 当前实现常量：本地最多 500 条 / 512KB，message 2KB，detail 8KB，feedback logs 256KB。
...
- 新增传输消息：`ATTACHMENT_CREATE`、`ATTACHMENT_APPEND_CHUNK`（base64）、`ATTACHMENT_FINALIZE`、`ATTACHMENT_READ_CHUNK`（base64）、`ATTACHMENT_ABORT`（按 `submitId` 或 explicit ids）。

### `adapters/types.ts`

- `subscribeToUserSubmissions?(onSubmit: (payload: { text; attachments; attachmentResolution?; onConsumed? }) => void)`。
- `getComposerAttachmentSnapshot?(capturedAttachments)`：source submit-time 当前附件快照，用于过滤 stale buffer。
- `prepareForDelivery?({ text, attachments, expectedSessionId, expectedUrl })`：target 注入前准备页面，适合 Manus clean composer 等操作。
- `setComposerPayload?({ text, attachments })`：渐进替代 `setComposerText`；adapter 可自定 text/attachment 顺序。
- `detectAttachmentUploadError?(): string | null | Promise<...>`：报告 provider 原生上传失败。
- `getComposerAttachmentPresence?(expectedAttachments): { count: number; keys?: string[] }`：报告当前 composer 附件 presence，供 controller 做 baseline+delta 确认。
- `suppressAttachmentCaptureFor?(durationMs)`：target 注入期间关闭 source capture，避免程序化 file input/change 反射。
- `ProviderAdapter.uploadCapability?: UploadCapability`。

### `runtime/attachment-store.ts`

background-owned，`chrome.storage.session` metadata + IndexedDB raw bytes。metadata 有 `status: writing | ready`，target 只能读 ready 附件。函数约：`createAttachment`（staging + reserve，**先 sweepExpired**，owner 取 `sender.tab?.id`）、`appendAttachmentChunk`、`finalizeAttachment`、`bindAttachments(submitId, workspaceId)`、`readAttachmentChunk`、`releaseSubmitAttachments(submitId)`、`abortAttachments(submitId | ids)`、`startupSweepAttachments`、`sweepAttachmentsByOwnerTab(tabId)`、`clearAllAttachments`、`getReservedAttachmentBytes`。
...
14. 动画 GIF/WebP 一家接受一家拒绝 → capability gating，无客户端转换。
15. 同图 attach 两次 → 存两份，按各自 id 独立删除。
16. SW restart mid-fan-out → TTL/startup sweep 清残留；orphan blob（无 metadata）startup 删除。
17. IndexedDB 写失败/配额超 → 同超预算处理：跳过 fan-out，移除该附件的部分 chunk。
18. dev "Clear persistent storage" → 同时清 session metadata + IndexedDB object store。
19. mixed 文本+内联 `<img>` 粘贴 → 跟随 provider 原生行为（实测只取文本），不解析。
...
- debug log 必须有大小边界：本地保留最近 tail，feedback 附带日志再用更小 budget 裁剪；截断只能标记过长字段，不能把有用事实改成隐私 placeholder。
