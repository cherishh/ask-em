...
type UploadCapability = {
  mimes: string[];
  extensions?: string[]; // 仅在 File.type 空/generic 时兜底
  maxFiles: number;
} | null; // null = 不支持附件
```

- 判定：`mime ∈ mimes || (type 空/generic 时) ext ∈ extensions`。
- mime 与 ext 都无法判定 → `unsupported-attachment`。
...
2. MIME/扩展名都不在 allowlist → `unsupported-attachment`，不做格式转换。
